...
```bash
# Copy folder
scp -i your-key.pem -r /path/to/your/project ec2-user@18.206.173.77:~/logic_monitor

# Or copy zip
scp -i your-key.pem /path/to/logic_monitor.zip ec2-user@18.206.173.77:~/
```

Connect to the server:

```bash
ssh -i your-key.pem ec2-user@18.206.173.77
cd ~/
unzip logic_monitor.zip   # Only if uploaded as zip
cd logic_monitor
```

> Replace `18.206.173.77` with your **private or public IP** depending on your environment.
...
```env
HOST_URL=https://18.206.173.77       # Use private or public IP
BACKEND_PORT=8000
DEBUG=False
ALLOWED_HOSTS=18.206.173.77,localhost
DJANGO_SECRET_KEY=your-secret-key

# Azure SSO redirect
AZURE_REDIRECT_URI=https://18.206.173.77:8000/oauth2/callback/
...
```env
REACT_APP_API_BASE_URL=https://18.206.173.77/api
REACT_APP_SCOPES=openid profile email offline_access User.Read
REACT_APP_APP_SCOPE=alerts
```

> Replace `18.206.173.77` with your instance’s IP (private or public depending on your setup).
...
| Component     | URL                                          |
| ------------- | -------------------------------------------- |
| Frontend SPA  | `https://18.206.173.77/`                          |
| Backend Admin | `https://18.206.173.77/admin/login/?next=/admin/` |
...
* **Backend redirect URI**: `https://18.206.173.77/oauth2/callback/`
